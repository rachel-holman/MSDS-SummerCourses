...
pctgenderGPA<-substudents%>%
  group_by(Gender, GPA.cat)%>%
  summarize(Counts=n())%>%
  mutate(Percent=Counts/nrow(substudents))

ggplot(pctgenderGPA, aes(x=GPA.cat, y=Percent, fill=Gender))+
  geom_bar(stat="identity", position="dodge")+
  theme(plot.title = element_text(hjust = 0.5))+
  labs(x="GPA Category", y="Proportion of Students", title="GPA Categories by Gender")
```
...
ggplot(genderGPApctSmoke, aes(x=GPA.cat, y=Percent, fill=Gender))+
  geom_bar(stat="identity", position="dodge")+
  facet_grid(~Smoke) +
  theme(plot.title = element_text(hjust = 0.5))+
  labs(x="GPA Category", y="Proportion of Students", title="GPA Categories by Gender and Smoking Status")
```
...
ggplot(fullstatedata, aes(x=Population,y=deaths, size=cases))+
  geom_point(alpha=0.5, color="blue")+
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))+
  labs(x="Population", y="Death Count", title="Scatterplot of Death Count against State Populations", subtitle = "Point Size Corresponds to Case Count")
```


The visual created above shows that there is a positive linear correlation between population size and death count. Additionally, the states with larger death counts also have a larger case count on average.
...
The correlation between body mass and flipper length for Gentoo penguins is 0.7026665.  
...
$Y = \beta_{0} + \beta_{1}X + e$ = $(body\_mass\_g) = -6787.281 + 54.623(flipper\_length\_mm) + 360.2$
...
$H_0$ : $\beta_1 = 0$ (The slope of the SLR is equal to 0)
$H_A$ : $\beta_1 \neq 0$ (The slope of the SLR is not equal to 0)
...
Since out test statistic is larger than the critical value, we reject the null hypothesis. Out data supports the claim that the slope is different from 0, or in other words, that there is a linear association between gentoo penguin flipper length (mm) and body mass (grams).
