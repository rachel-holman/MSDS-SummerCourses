...
# Student Info
    ...
    https://github.com/rachel-holman/DS5100-dnw9qk/blob/main/lessons/M10/hw10.Rmd
...
```{r}
pct_quota = c(.40,.30,.20,.10)
all_quota = cumsum(pct_quota)
pct_earned = c(.07,.10,.13,.16)

q_earnings = 225000*pct_quota
q_payout = 225000*pct_quota*pct_earned

all_earnings = cumsum(q_earnings)
all_payout = cumsum(q_payout)

df <- data.frame(pct_quota,pct_earned,q_earnings,q_payout, 
                 all_quota, all_earnings, all_payout)
df
```
...
```{r}
payout_earned <- function(frac){
  # this computes payout values
  if (frac <= .40) {
    amount <- 225000*frac*df$pct_earned[1]
  } else if (frac <= .70) {
    amount <- df$all_payout[1] + 225000*(frac-df$all_quota[1])*df$pct_earned[2]
  } else if (frac <= .90) {
    amount <- df$all_payout[2] + 225000*(frac-df$all_quota[2])*df$pct_earned[3]
  } else {
    amount <- df$all_payout[3] + 225000*(frac-df$all_quota[3])*df$pct_earned[4]
  }
}
```
...
```{r}
pct_quotas = seq(0,1,0.1)
all_payouts = unlist(lapply(pct_quotas, payout_earned))

df2 <- data.frame(pct_quotas, all_payouts)
df2
```
...
```{r}
library(ggplot2)
ggplot(data=df, aes(x=all_quota, y=all_earnings, group=1)) +
  geom_line()+
  geom_point()+
  labs(title="Amounts Earned vs. Quarterly Quota Rached",
       x="Quarterly Quota Reached", y = "Amount Earned (USD)")+
  theme_bw()
```
...
```{r}
ggplot(data=df2, aes(x=pct_quotas, y=all_payouts, group=1)) +
  geom_line()+
  geom_point()+
  labs(title="Payout Amount vs. Percent Quota Rached",
       x="Percent Quota Reached", y = "Payout Amount (USD)")+
  theme_bw()
```